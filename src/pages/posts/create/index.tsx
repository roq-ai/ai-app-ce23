...
function PostCreatePage() {
  ...
  const formik = useFormik<PostInterface>({
    initialValues: {
      content: '',
      scheduled_at: new Date(new Date().toDateString()),
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="scheduled_at" mb="4">
            <FormLabel>Scheduled At</FormLabel>
            <Box display="flex" maxWidth="100px" alignItems="center">
              <DatePicker
                dateFormat={'dd/MM/yyyy'}
                selected={formik.values?.scheduled_at ? new Date(formik.values?.scheduled_at) : null}
                onChange={(value: Date) => formik.setFieldValue('scheduled_at', value)}
              />
              <Box zIndex={2}>
                <FiEdit3 />
              </Box>
